...
middle position of this linked list and print all elements. 
...
void insertAtMiddle(struct Node** flag, int newData) {
    struct Node* newNode = (struct Node*)malloc(sizeof(struct Node));
    ...
    struct Node* slow = (*flag)->next;
    struct Node* fast = (*flag)->next;

    while (fast->next != *flag && fast->next->next != *flag) {
        slow = slow->next;
        fast = fast->next->next;
    }
...
int main() {
    ...
    int newValue;
    printf("\nEnter the value to insert at the middle: ");
    scanf("%d", &newValue);
    insertAtMiddle(&flag, newValue);

    printf("\nCircular linked list after inserting at the middle:\n");
    printList(flag);
